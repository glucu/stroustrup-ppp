...
protected:
	Shape() { }
	Shape(initializer_list<Point> lst) { for(Point p : lst) add(p); }  // add() the Points to this Shape
...
public:
    ...
    virtual void print() const { std::cout << "Shape::print()\n"; }
...
private:
	...
	Color lcolor {int(fl_color())}; // cast was used to supress narrowing warning
	Line_style ls {0};
...
struct Circle : Shape {
	...
	void draw_lines() const;
    void print() const override { std::cout << "Circle::print()\n";}
...
struct Ellipse : Shape {
	Ellipse(Point p, int ww, int hh)	// center, max, and min distance from center
	:w{ ww }, h{ hh } {
