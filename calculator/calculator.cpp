 ...
 * Revision history:
 *
 *     Originally written by glucu Fall 2023
 *     Revised by glucu Dec. 2023
 *     1. Modified grammar to include exponent/HiOp production rule
 *     2. Add (underscore, assignment, constants)
 *     3. Defined class Symbol_table
 *     4. help guide for instructions
 *     Revised by glucu Jan. 2024
 *     1. change factorial from recursive to iteration
 *
 *
 *     Future improvements and implementations
 *     1. Use a vector of all the keywords instead of symbolic constants (this can help scale the program better)
 *     2. separate compilation: splt code into header files for better code organization
 *     3. Use a map for the symbol table*
 *     4. Implement a command that to print the symbol table
 *
 *
...
public: 
    ...
    Token_stream(istream &);                 // reads from an istream
...
// return true if overflow happened; otherwise return false
{
    return 0 > numeric_limits<int>::max() - n;
}
...
{
    if(n < 0) error("factorial: Factorial is only defined for non-negative real numbers");
    int res = 1;
    while(n > 1) {
        res *= n--;
        if(factorial_overflow(res)) error("factorial: overflow!");
    }
    return res;
}
